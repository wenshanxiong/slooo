...
def cpu_slow(slow_server_config, slow_ip, slow_pids, fault_level):
    quota = fault_level.get('cpu_quota', 50000)
    period = 1000000
...
def cpu_contention(slow_server_config, slow_ip, slow_pids, fault_level):
    ...
    contention_process = fault_level.get('cpu_contention_process', 'resources/slowness/deadloop')
    slow_cpu_share = fault_level.get('slow_cpu_share', 64)
    contention_process_name = contention_process.split('/')[-1]
    scp @(contention_process) @(slow_ip):~/
    ssh -i ~/.ssh/id_rsa @(slow_ip) f"sh -c 'nohup taskset -ac {cpu} ./{contention_process_name} > /dev/null 2>&1 &'"
    deadlooppid=$(ssh -i ~/.ssh/id_rsa @(slow_ip) "sh -c 'pgrep {}'".format(contention_process_name))
    ssh -i ~/.ssh/id_rsa @(slow_ip) "sudo sh -c 'sudo mkdir /sys/fs/cgroup/cpu/cpulow /sys/fs/cgroup/cpu/cpuhigh'"
    ssh -i ~/.ssh/id_rsa @(slow_ip) "sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/cpulow/cpu.shares'".format(slow_cpu_share)
...
def disk_slow(slow_server_config, slow_ip, slow_pids, fault_level):
    ...
    lsblkcmd = fault_level.get('lsblkcmd', "8:32 524288")
...
def network_slow(slow_server_config, slow_ip, slow_pids, fault_level):
    delay = fault_level.get('delay', '400ms')
...
def memory_contention(slow_server_config, slow_ip, slow_pids, fault_level):
    mem_quota = fault_level.get('mem_quota', 5 * 1024 * 1024) # 5MB by default
...
def fault_inject(exp, server_config, pids, snooze, fault_level):
    ...
    if exp == "kill":
        ...
        slow_vs_num[int(exp)](server_config, ip, pids, fault_level[exp])
